...
public class Bubbles implements Screen {
	...
	public void show() {
		...
		world.setContactListener(new ContactListener() {
			...
			public void beginContact(Contact contact) {
				LinkedList fa = (LinkedList)(contact.getFixtureA().getUserData());
				LinkedList fb = (LinkedList)(contact.getFixtureA().getUserData());

				if (fa.get(0) == "Player" && fb.get(0) == "Bubble"){
					contact.getFixtureA().getBody().applyForceToCenter(new Vector2(
							parseFloat((String)fa.get(1)) - parseFloat((String)fb.get(1)),
							parseFloat((String)fa.get(2)) - parseFloat((String)fb.get(2))
					).scl(PUSH_SPEED), true);
				}
				if (fb.get(0) == "Player" && fa.get(0) == "Bubble"){
					contact.getFixtureB().getBody().applyForceToCenter(new Vector2(
							parseFloat((String)fb.get(1)) - parseFloat((String)fa.get(1)),
							parseFloat((String)fb.get(2)) - parseFloat((String)fa.get(2))
					).scl(PUSH_SPEED), true);
				}
			}
	...
	public void updateRadar() {
		...
		final float OBJECT_RADIUS = RADIUS / 20;
		...
		for (Bubble bubble : b) {
			...
			if (dist < stage.getWidth() / 5) {
				...
				renderer.circle(CENTER_X + dist * (float) -Math.cos(theta), CENTER_Y + dist * (float) -Math.sin(theta), OBJECT_RADIUS);
			}
		}
		...
		// player indicator
		renderer.setColor(Color.valueOf("#bbbbbb"));
		renderer.circle(CENTER_X, CENTER_Y, OBJECT_RADIUS + 2.f);

		renderer.setColor(Color.valueOf("#cccccc"));
		renderer.circle(CENTER_X, CENTER_Y, OBJECT_RADIUS + 1.f);

		renderer.setColor(Color.valueOf("#dddddd"));
		renderer.circle(CENTER_X, CENTER_Y, OBJECT_RADIUS);

		renderer.setColor(Color.valueOf("#eeeeee"));
		renderer.circle(CENTER_X, CENTER_Y, OBJECT_RADIUS - 1.f);
